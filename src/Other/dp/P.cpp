...
m64 dfs(i64 color, i64 idx, i64 from) {
    if (dp[color][idx]) return *dp[color][idx];

    m64 res = 1;
    for (auto& nxt : g[idx]) {
        if (nxt == from) continue;
        res *= dfs(0, nxt, idx) + dfs(1, nxt, idx) * !color;
    }
    return dp[color][idx].emplace(res);
}
...
void solve() {
    ...
    dp.resize(2, vector<optional<m64>>(n));
    cout << dfs(0, 0, -1) + dfs(1, 0, -1) << endl;
}
